...
router.post('/login', async (req, res) => {
  ...
  try {
      ...
      if (existingOTP) {
          ...
          await OTP.findByIdAndUpdate(existingOTP._id, { otp: hashedOTP });
      } else {
          // Create a new OTP record
          await OTP.create({name: 'login_otp', userId: user._id, email: email, otp: hashedOTP });
      }

      // Send the OTP to the user's email (implement this function)
      sendVerificationCode(user.email, otp);

      // Return success response
      return res.status(200).json({ success: "Verification code sent successfully", data: token });
  } catch (error) {
      console.log(error);
